...
class Con_AbsenceTrackingReporting extends CI_Controller {
    ...
    public function index($menu_id=0, $show_result = FALSE, $search_data = array(), $search_criteria = array('from_date'=>'','to_date'=>'','acc_type' => '','employee_name' => '','department_id' => '','absent_type' => '')) {
        ...
        $param['page_header'] = "Employee Absence Tracking  Reporting";
        ...
        $param['content'] = 'report/view_EmployeeAbsentTrackigReporting.php';
        $this->load->view('admin/home', $param);
    }
    
    public function search_AbasentTrackingReporting() {
        ...
        $search_criteria['acc_type'] = $acc_type = $this->input->post('acc_type');
        ...
        if (($from_date!='' && $to_date!='') ||$employee_name != '' || $acc_type != '' || $department_id != '' || $absent_type != '') {
            $this->db->select('main_employees.employee_id,'
                    ...
                    . 'main_emp_absencetracking.absent_type,'
                    . 'main_emp_absencetracking.from_date,'
                    . 'main_emp_absencetracking.to_date,'
                    . 'main_emp_absencetracking.total_days,'
                    . 'main_emp_absencetracking.details_reason,'
                    . 'main_emp_absencetracking.is_leave,'
                    . 'main_emp_absencetracking.id,'
                    . 'main_employees.isactive,'
                    . 'main_emp_workrelated.department,'
                    . 'main_employees.salutation');
            $this->db->from('main_employees');
            $this->db->join('main_emp_absencetracking', 'main_emp_absencetracking.employee_id = main_employees.id','LEFT');
            ...
            if($from_date!='' && $to_date!=''){
                ...
                $this->db->where("main_emp_absencetracking.from_date BETWEEN '".$from_date."' AND '".$to_date."'");
                // $this->db->where('main_emp_actions.action_date <=',$to_date);                
            }
   
            if ($employee_name != '') {
                $this->db->where('main_employees.employee_id', $employee_name); //main_emp_absencetracking
            }
            if ($department_id != '') {
                $this->db->where('main_emp_workrelated.department', $department_id);
            }
            if ($absent_type != '') {
                $this->db->where('main_emp_absencetracking.absent_type', $absent_type);
            }
