...
class Con_Interview_Schedule_Reporting extends CI_Controller {
    ...
    public function index($menu_id=0, $show_result = FALSE, $search_data = array(), $search_criteria = array('action_from' => '','action_to' => '','requisition_id' => '','interview_type' => '','interview_status' => '')) {
        ...
        $param['content'] = 'talentacquisition/view_Interview_Schedule_Reporting.php';
        $this->load->view('admin/home', $param);
    }
    
    public function search_Interview_Schedule_Reporting() {
        ...
        $search_criteria['interview_type'] = $interview_type = $this->input->post('interview_type');
        $search_criteria['interview_status'] = $interview_status = $this->input->post('interview_status');

        if (($action_from != '') || ($action_to != '') || ($requisition_id != '') || ($interview_type != '') || ($interview_status != '') ) {
            ...
            /* ----Conditions---- */
            if ($action_from != '') {
                $this->db->where('interview_date >=', $this->Common_model->convert_to_mysql_date($action_from));
            }
            if ($action_to != '') {
                $this->db->where('interview_date <=', $this->Common_model->convert_to_mysql_date($action_to));
            }
            if ($requisition_id != '') {
            ...
            if ($interview_status != '') {
                $this->db->where('interview_status', $interview_status);
            }
