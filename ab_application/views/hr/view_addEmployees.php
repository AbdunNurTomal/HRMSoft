...
<script type="text/javascript">
    ...
    //MHK
    let hasBtnEmployeeNoteClose = false;
</script>   
                ...
                <div class="row tab-v3">
                    <div class="col-sm-2" id="tabs"> <!-- required for floating -->
                        ...
                        <ul class="nav nav-tabs nav-pills nav-stacked">
                            ...
                            <li class=""><a href="#WageCompensation" data-toggle="tab"><i class="fa fa-chevron-circle-right"></i> Wage History Report </a></li>
                            ...
                            <?php 
                            if( $page_id == 'edit_entry' )
                            {
                            	echo '<li class=""><a href="#add_note"  data-toggle="tab"><i class="fa fa-chevron-circle-right"></i> Add Note</a></li>';
//                             	echo '<li class=""><a href="#view_note" data-toggle="tab"><i class="fa fa-chevron-circle-right"></i> View Note</a></li>';
                            	echo '<li class=""><a href="'.base_url().'Con_Employee_Note/show_notes_by_employee/'.$employee_id.'"><i class="fa fa-chevron-circle-right"></i> View Note</a></li>';
            				}
                            ?>
                    ...
                    <div class="col-sm-10">
                        ...
                        <div class="tab-content tag-box tag-box-v3">
                            ...
                            if( $page_id == 'edit_entry' )
                            {
                            	echo '<div class="tab-pane" id="add_note">' ; include_once( "tab/add_note.php"  ); echo '</div>';
                            	echo '<div class="tab-pane" id="view_note"></div>';
            				}
                            ?>
...
<script>
    ...
    $(function () {
        ...
        $("#sky-form11").submit(function (event) {
            ...
            console.error(url);
            console.error($("#sky-form11").serialize());
            console.error($(this).attr('method'));
            $.ajax({
                url: url,
                ...
                $("#wage_compensation_div").load(location.href + " #wage_compensation_div");
                ...
                reload_table('dataTables-example-compensation');
                ...
                if(datas[4])
                {
                    ...
                    getJWT(clint_accountant, clint_site_id,datas[1], clint_api_secret, function
                    (err, jwt) {
                    if (err) {
                        ...
                        //$('#processingText').text('Please use the web clock below');10
                        console.log('Token: ' + jwt);
                        if(datas[5]==1)
                        {
                            //getCreateEmployee(jwt,empdata);
                            getCreateEmployee(jwt,datas[4]);
                        }
                        if(datas[5]==2)
                        {
                            getUpdateEmployee(jwt,datas[1],datas[4]);
                        }
...
function getCreateEmployee(jwt,datas) {
          
        //        var datas = {
        //            "EmployeeCode": "890009",
        //            "FirstName": "SOHEL BIJAY",
        //            "LastName": "SOHEL BIJAY",
        //            "Designation": "API SOHEL BIJAY",
        //            "Phone": "01730082104",
        //            "Email": "sohelbijay@gmail.com",
        //          }
        //
        //alert (jwt);
        //alert (datas);
        ...
        $.ajax({
            url: "https://twpapi.payrollservers.us/api/"+ clint_site_id +"/employees",
            //url: "https://twpapi.payrollservers.us/swagger/ui/index#!/Employees",
            headers: {
               "Authorization": 'Bearer ' + jwt,
				"Content-Type": "application/json"
            },
...
function getUpdateEmployee(jwt,emp_code,datas) {
  
        $.ajax({
            //url: "https://twpapi.payrollservers.us/api/33172/employees",
            url: "https://twpapi.payrollservers.us/api/"+ clint_site_id +"/employees/"+emp_code+"?idType=employeeCode",
            //uri: `${CONFIG.twpApiURL}/${CONFIG.siteId}/employees/${employeeCode}?idType=employeeCode`,
            //url: "https://twpapi.payrollservers.us/swagger/ui/index#!/Employees",
            headers: {
               "Authorization": 'Bearer ' + jwt,
				"Content-Type": "application/json"
...
function get_data(emp_id){
    ...
    var dates = $('[name="hire_date"]').val().split("-");
    var hire_date= dates[2] + "-" + dates[0]+"-"+dates[1];

    var data={
        "EmployeeCode": emp_id,
        "FirstName": $('[name="first_name"]').val(),
        "MiddleName": $('[name="middle_name"]').val(),
        "LastName": $('[name="last_name"]').val(),
        "Designation": $('[name="position"]').val(),
    ...
    function edit_wage_compensation(id) {
        ...
        $.ajax({
            ...
            success: function (data)
            {
                ...
                $('[name="emp_wage_position"]').select2('val', data.position);
                if (data.pay_schedule) {
                    $('[name="pay_schedule"]').select2('val', data.pay_schedule);
                }
                $('[name="wage_salary_type"]').val("");
                $('#wage_salary_type_val').html("");
                $('#wage_salary_type_val').html(data.wage_salary_type=='0'? "Salary" : "Hourly");
                $('[name="wage_salary_type"]').val(data.wage_salary_type);

                //$('[name="wage_salary_type"]').select2('val', data.wage_salary_type);
                //change_salary_rate(data.wage_salary_type);
                //$('[name="hours_per_pay_period"]').val(data.hours_per_pay_period);

                //if(data.wage_salary_type==0){
                    $('[name="yearly_salary"]').val(data.yearly_salary);
                //}else{
                    $('[name="per_hour_rate"]').val(data.per_hour_rate);
               // }

              // alert($("#wage_salary_type").val());
                ...
                //$('[name="wage_status"]').select2('val', data.status);

                $('[name="effective_date"]').val(show_date_formate_js(data.effective_date));
...
//                $('[name="em_relationship"]').val(data.relationship);
    ...
    $(function () {
        $("#emp_Disc").submit(function (event) {
            var url;
            if (save_method == 'add')
            {
                url = "<?php echo site_url('Con_Employees/save_emp_disc_actions') ?>";
            } else
            {
                url = "<?php echo site_url('Con_Employees/edit_emp_disc_actions') ?>";
            }
            $.ajax({
                url: url,
                data: $("#emp_Disc").serialize(),
                type: $(this).attr('method')
            }).done(function (data) {
    ...
    function edit_action(id) {
        ...
        $.ajax({
            ...
            success: function (data)
            {
                ...
                else if (data.action_type == 3)  /* discipline */
                {
                    $('[name="id_emp_disc"]').val(data.id);
                    // $('[name="inc_action_date"]').val(show_date_formate_js(data.action_date));
                    // $('[name="incident_category"]').select2().select2('val', data.incident_category);
                    // $('[name="tncident_type"]').select2().select2('val', data.tncident_type);
                    ...
                    if (data.discipline_type)
                    {
                        $('[name="disc_discipline_type"]').select2().select2('val', data.discipline_type);
                        //change_inc_data(data.discipline_type);
                    }
                    ...
                    $('#Disc_Modal').modal('show'); // show bootstrap modal when complete loaded
                    $('.modal-title').text('Edit Discipline'); // Set title to Bootstrap modal title
                } else  /* Accident */
                {
