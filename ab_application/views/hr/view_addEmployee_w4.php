<div class="col-md-10 main-content-div">
    <div class="main-content">
        ...
        <div class="container tag-box tag-box-v3"> <!-- container well div -->
            ...
            if ($type == 1) {//entry
                ?>
                <form id="sky-form11" name="sky-form11" class="form-horizontal" method="post" action="<?php echo base_url() . 'Con_Employee_w4/save_obw4'; ?>" enctype="multipart/form-data" role="form">
                        ...
                        <div class="form-group margin-top-15">
                            ...
                            <div class="col-sm-3 no-padding">
                                <select name="ob_eeo_emp_id" id="ob_eeo_emp_id" class="col-sm-12 col-xs-12 myselect2 input-sm" >
                                    <option></option>
                                    <?php
                                    $employees_candidate = $this->db->get_where('main_employees', array('company_id' => $this->company_id));
                                    if ($employees_candidate && ($employees_candidate->num_rows() > 0)) {
                                        foreach ($employees_candidate->result() as $key => $row) {
                ...
                $obw4_data = $query->row();
                ?>
                <form id="sky-form11" name="sky-form11" class="form-horizontal" method="post" action="<?php echo base_url() . 'Con_Employee_w4/update_obw4/' . $obw4_data->id; ?>" enctype="multipart/form-data" role="form">
                        ...
                        <div class="form-group margin-top-15">
                            ...
                            <div class="col-sm-3 no-padding">
                                <select name="ob_eeo_emp_id" id="ob_eeo_emp_id" class="col-sm-12 col-xs-12 myselect2 input-sm" disabled>
                                    ...
                                    if ($employees_candidate && ($employees_candidate->num_rows() > 0)) {
                                        foreach ($employees_candidate->result() as $key => $row) {
                                            $slct = ($obw4_data->employee_id == $row->employee_id) ? 'selected' : '';
...
<script type="text/javascript">
    ...
    $(function () {
        $("#sky-form11").submit(function (event) {
            ...
            $.ajax({
                ...
                var url = '<?php echo base_url() ?>Con_Employee_w4';
    ...
    $('#ob_eeo_emp_id').on('change', function () {
        ...
        $.ajax({
            url: "<?php echo site_url('Con_Employee_w4/ajax_get_candidate_data/') ?>/" + $ob_employee_id,
            type: "GET",
            dataType: "JSON",
            success: function (data)
            {
                ...
                var Loc = [data.city, data.state, data.zipcode];
