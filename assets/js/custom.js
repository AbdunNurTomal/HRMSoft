...
function view_message(data, url, modal_id, form_id) {
    ...
    {
        swal("Warning!", "No Message Found", "warning")
        .then(function(value) {
            $('#messagebox').empty();
            $('html, body').animate({scrollTop: 0}, 800);
        })
...
function view_message_backup(data, url, modal_id, form_id) {
    ...
    if (data)
    {
        ...
        if (datas[1] == 1)//Successful==1
        {
            $("#messagebox").fadeTo(100, 0.1, function () {
                $(this).html(datas[0]).fadeTo(400, 1);
    ...
    {
        $("#messagebox").fadeTo(100, 0.1, function () {
            $(this).html('No Message Found').fadeTo(400, 1);
            $('html, body').animate({scrollTop: 0}, 400);
            //$(this).fadeOut(5000);
        });
    }
...
/* $(document).on("click", "#btnSendSMS", function() {
    var selectedEmployee = $("#contact-employee-select").val();
    ...
    $.ajax({
        url: url,
        data: { employee_id: selectedEmployee, message: message },
        method: "POST",
        dataType: "JSON",
        success: function(response) {
            // console.log(response.msg);
            if (response.status) {
                $("#contact-employee-alert").html(response.msg).removeClass("alert-danger").addClass("alert-success").show().fadeOut(10000);
            } else {
                $("#contact-employee-alert").html(response.msg).removeClass("alert-success").addClass("alert-danger").show().fadeOut(10000);
            }
        },
        error: function(err) {
